...
class CartController {
    async createCart(req, res) {
        ...
        if (cart) {
            ...
            cart.totalPrice = product.productPrice * cart.quantity;
            await cart.save();
        } else {
            await modelCart.create({ userId: id, productId, quantity, totalPrice: product.productPrice * quantity });
        }
        new OK({
    ...
    async updateQuantity(req, res) {
        const { id } = req.user;
        const { productId, quantity, fullName, phoneNumber, address, nameCoupon } = req.body;
        ...
        if (nameCoupon) updateData.nameCoupon = nameCoupon;
